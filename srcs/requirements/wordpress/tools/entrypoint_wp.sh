...
if [ ! -f /var/www/html/wp-config.php ]; then
	...
	wp --allow-root config create --dbname=$WORDPRESS_DB_NAME --dbuser=$WORDPRESS_DB_USER --dbpass=$WORDPRESS_DB_PASSWORD --dbhost=$MYSQL_HOST;
else
	echo "wp-config.php already exists !"
fi
echo "Installing the core..."
wp --allow-root core install --url=$WORDPRESS_URL --title=$WORDPRESS_TITLE --admin_email=$WORDPRESS_ADMIN_EMAIL --admin_user=$WORDPRESS_ADMIN_LOGIN --admin_password=$WORDPRESS_ADMIN_PASSWORD;
#wp --allow-root plugin install redis-cache --activate;
echo "Creating user..."
wp --allow-root user create $WORDPRESS_USER_LOGIN $WORDPRESS_USER_EMAIL --user_pass=$WORDPRESS_USER_PASSWORD 2> /dev/null;
if [ $? -eq 1 ]; then
